...
//				this.fadeOut(loading$);
			...
			/*
			 * 1.相邻的非0且与起点相同的点放入结果列表，将为0的放入开放列表
			 * 2.搜索开放列表中点的相邻点，将非0点且与起点相同的点放入结果列表
			 * 3.搜索下一个为0的
			 * */
			canCleanup:function(x1,y1,x2,y2){	//广度优先搜索
				var safe = 0, result = [], open = [[x1,y1]], close = [],
					final_point = this.mapArray[y2][x2],
					width = this.map_width,
					height = this.map_height,
					turn = 0;

				while(this.indexOf(result,[x2,y2]) < 0 && turn < 3 && safe < 50){
					var tmp = [],
						direct = "x";

					while(open.length > 0 && this.indexOf(result,[x2,y2]) < 0){
						var father_point = open.shift(),	//取开放列表中的第一个点
							x = father_point[0],
							y = father_point[1],
							son_point,
							current_direct;
	
						if(x+1 < width && this.indexOf(result,[x+1,y]) < 0 && this.indexOf(close,[x+1,y]) < 0){
							son_point = this.mapArray[y][x+1];
							current_direct = "x";
							if(current_direct != direct){
								turn++;
							}
							if(turn < 3){
								if(son_point === final_point){
									result.push([x+1,y]);	//加入可连通的列表
								} else if(son_point === 0){
									tmp.push([x+1,y]);	//加入开放列表
								} else {
									close.push([x+1,y]);	//加入以检测列表
								}
							}
						}
						
						if(this.indexOf(result,[x2,y2]) >= 0){
							return true;
						}

						if(x-1 >= 0 && this.indexOf(result,[x-1,y]) < 0 && this.indexOf(close,[x-1,y]) < 0){
							son_point = this.mapArray[y][x-1];
							current_direct = "x";
							if(current_direct != direct){
								turn++;
						...
						if(y+1 < height && this.indexOf(result,[x,y+1]) < 0 && this.indexOf(close,[x,y+1]) < 0){
							son_point = this.mapArray[y+1][x];
							current_direct = "y";
							if(current_direct != direct){
								turn++;
							}
						...
						if(y-1 >= 0 && this.indexOf(result,[x,y-1]) < 0 && this.indexOf(close,[x,y-1]) < 0){
							son_point = this.mapArray[y-1][x];
							current_direct = "y";
							if(current_direct != direct){
								turn++;
				...
				if(this.indexOf(result,[x2,y2]) >= 0){	//可以连通
					return true;
				}
